...
#Save
saveRDS(GSEAres_all, file = "results/Pt9/GSEA_pt9_BL_REL_LMPP_MPP_all_dream.rds")
data.table::fwrite(GSEAres_all, file ="results/Pt9/GSEA_pt9_BL_REL_LMPP_MPP_all_dream.tsv" , sep = "\t", sep2 = c("", " ", ""))
...
###############################################################################################
#The expression cutoff at 1 greatly reduces the genes which are analysed, therefore I try to adjsut the threshold a bit
#Open the annotated Seurat object
df<-readRDS("data/4_Seurat_obj_CNA_referenceannotations.rds")
library(tidyverse)
...
# Filter out genes expressed in fewer cells than min.expFraction, here I use 0.5 as the cutoff and not 1 as above
...
z0<-read_csv("results/Pt9/DREAM DEA/LMPP_MPP/Pt9_DREAM_LMPP_MPP_exp0.5.csv")
...
## 1. Read in data -----------------------------------------------------------
list.files(in_path)
df_new <- read.csv("results/Pt9/Pt9_DREAM_LMPP_MPP_exp0.5.csv", row.names = 1)
## 2. Prepare background genes #leave this out if already done -----------------------------------------------
# Download gene sets .gmt files
#https://www.gsea-msigdb.org/gsea/msigdb/collections.jsp
# For GSEA
...
#Check whether it worked
head(GSEAres_GO)
write_csv(GSEAres_GO, "results/Pt9/GSEA_LMPP_MPP_pt9_GO_Dream_exp0.5.csv")
...
#Save
## 5. Save the results -----------------------------------------------
saveRDS(GSEAres_GO, file = "results/Pt9/GSEA_pt9_BL_REL_LMPP_MPP_GO__exp0.5dream.rds")
data.table::fwrite(GSEAres_GO, file ="results/Pt9/GSEA_pt9_BL_REL_LMPP_MPP_GO_dream_exp0.5.tsv" , sep = "\t", sep2 = c("", " ", ""))
...
#Check whether it worked
head(GSEAres_HALLMARK)
write_csv(GSEAres_HALLMARK,"results/Pt9/GSEA_pt9_LMPP_MPP_HALLMARK_dream_exp0.5.csv")
...
plotEnrichment(bg_genes_HALLMARK[[head(GSEAres_HALLMARK[order(padj), ], 1)$pathway]],
  ...
  labs(title = head(GSEAres_HALLMARK[order(padj), ], 1)$pathway)
#Save
saveRDS(GSEAres_HALLMARK, file = "results/Pt9/GSEA_pt9_BL_REL_LMPP_MPP_HALLMARK_dream_exp0.5.rds")
data.table::fwrite(GSEAres_HALLMARK, file ="results/Pt9/GSEA_pt9_BL_REL_LMPP_MPP_HALLMARK_dream_exp0.5.tsv" , sep = "\t", sep2 = c("", " ", ""))
#Visualise
GSEAres_HALLMARK$adjPvalue <- ifelse(GSEAres_HALLMARK$padj <= 0.05, "significant", "non-significant")
cols <- c("non-significant" = "grey", "significant" = "red")
GSEAres_HALLMARK_sig=subset(GSEAres_HALLMARK, GSEAres_HALLMARK$padj<0.05)
...
deseq<-read_csv("/Users/rabeamecklenbrauck/Library/CloudStorage/OneDrive-Nexus365/Ivo_Ven_Resistance mechanisms/Transcriptome analysis/Patient 9/DeSeq2/MPP_LMPP/DEA_pt9_LMPP_MPP_padj.csv")
