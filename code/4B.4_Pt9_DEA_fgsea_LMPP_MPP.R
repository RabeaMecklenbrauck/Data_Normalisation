...
## 1. Read in data -----------------------------------------------------------
list.files(in_path)
df_new <- read.csv("results/Pt9/DEA/LMPP_MPP/DEA_pt9_LMPP_MPP_padj.csv", row.names = 1)
...
write_csv(GSEAres_HALLMARK,"results/Pt9/GSEA_pt9_LMPP_MPP_Hallmark.csv")
...
#Check INF gamma
plotEnrichment(bg_genes_HALLMARK$HALLMARK_INTERFERON_GAMMA_RESPONSE, rankings) + 
  labs(title ="INF gamma response")

#Try to make a heatmap of the expression
library(RColorBrewer)
...
gmt_files <- list.files(path = bg_path, pattern = '.gmt', full.names = TRUE)
gmt_files
gmt.file <- system.file("")
library(ComplexHeatmap)
?heatmap.2
library(gplots)
gs.sub<-gmt_files[70]
gs.sub
heatmap.2( assay(vst)[bg_genes_HALLMARK$HALLMARK_INTERFERON_GAMMA_RESPONSE,], scale="row", 
           trace="none", dendrogram="column", 
           col = colorRampPalette( rev(brewer.pal(9, "RdBu")) )(255),
           ColSideColors = c(baseline="darkgreen", relapse = "darkred")[
             colData(vst)$condition ] )
...
#Save
saveRDS(GSEAres_HALLMARK, file = "results/Pt9/GSEA_pt9_BL_REL_LMPP_MPP_HALLMARK.rds")
data.table::fwrite(GSEAres_HALLMARK, file ="results/Pt9/GSEA_pt9_BL_REL_LMPP_MPP_HALLMARK.tsv" , sep = "\t", sep2 = c("", " ", ""))
...
gmt_files
#according to this list KEGG = [11], GO = [53], reactome = [17], wikipathways [21], misigdb [69], this script uses KEGG, GO and Misigdb
bg_genes_C2 <- prepare_gmt(gmt_files[5], my_genes, savefile = FALSE)

#Run GSEA
GSEAres_C2 <- fgsea(pathways = bg_genes_C2, # List of gene sets to check
                          stats = rankings,
                          scoreType = 'std', # in this case we have both pos and neg rankings. if only pos or neg, set to 'pos', 'neg'
                          minSize = 10,
                          maxSize = 500,
...
#Check whether it worked
head(GSEAres_C2)
write_csv(GSEAres_C2,"results/Pt9/GSEA_pt9_LMPP_MPP_all.csv")

#Order by pvalue
head(GSEAres_C2[order(pval),])
sum(GSEAres_C2[,padj<0.05])
sum(GSEAres_C2[,pval<0.01])


# plot the most significantly enriched pathway
plotEnrichment(bg_genes_C2[[head(GSEAres_C2[order(padj), ], 1)$pathway]],
               rankings) + 
  labs(title = head(GSEAres_C2[order(padj), ], 1)$pathway)
#Save
saveRDS(GSEAres_C2, file = "results/Pt9/GSEA_pt9_BL_REL_LMPP_MPP_all.rds")
data.table::fwrite(GSEAres_C2, file ="results/Pt9/GSEA_pt9_BL_REL_LMPP_MPP_all.tsv" , sep = "\t", sep2 = c("", " ", ""))

#Visualise
library(cowplot)
GSEAres_C2$adjPvalue <- ifelse(GSEAres_C2$padj <= 0.05, "significant", "non-significant")
cols <- c("non-significant" = "grey", "significant" = "red")
GSEAres_C2_sig=subset(GSEAres_C2, GSEAres_C2$padj<=0.1)
ggplot(GSEAres_C2_sig, aes(reorder(pathway, NES), NES, fill = adjPvalue)) +
  geom_col() +
  scale_fill_manual(values = cols) +
  theme(axis.text.x = element_text(angle = 90, vjust = 0.5, hjust=1)) +
  coord_flip() +
  labs(x="Pathway", y="Normalized Enrichment Score",
       title="Transcription factor pathways Enrichment Score from GSEA (p<0.1)")

#Oncogenic signatures
gmt_files <- list.files(path = bg_path, pattern = '.gmt', full.names = TRUE)
gmt_files
#according to this list KEGG = [11], GO = [53], reactome = [17], wikipathways [21], misigdb [69], this script uses KEGG, GO and Misigdb
bg_genes_onco <- prepare_gmt(gmt_files[58], my_genes, savefile = FALSE)

#Run GSEA
GSEAres_onco <- fgsea(pathways = bg_genes_onco, # List of gene sets to check
                    stats = rankings,
                    scoreType = 'std', # in this case we have both pos and neg rankings. if only pos or neg, set to 'pos', 'neg'
                    minSize = 10,
                    maxSize = 500,
...
#Check whether it worked
head(GSEAres_onco)
write_csv(GSEAres_onco,"results/Pt9/GSEA_pt9_LMPP_MPP_onco.csv")

#Order by pvalue
head(GSEAres_onco[order(pval),])
sum(GSEAres_onco[,padj<0.05])
sum(GSEAres_onco[,pval<0.01])


# plot the most significantly enriched pathway
plotEnrichment(bg_genes_onco[[head(GSEAres_onco[order(padj), ], 1)$pathway]],
               rankings) + 
  labs(title = head(GSEAres_onco[order(padj), ], 1)$pathway)
#Save
saveRDS(GSEAres_onco, file = "results/Pt9/GSEA_pt9_BL_REL_LMPP_MPP_onco.rds")
data.table::fwrite(GSEAres_onco, file ="results/Pt9/GSEA_pt9_BL_REL_LMPP_MPP_onco.tsv" , sep = "\t", sep2 = c("", " ", ""))

#Visualise
library(cowplot)
GSEAres_onco$adjPvalue <- ifelse(GSEAres_onco$padj <= 0.05, "significant", "non-significant")
cols <- c("non-significant" = "grey", "significant" = "red")
GSEAres_onco_sig=subset(GSEAres_onco, GSEAres_onco$padj<=0.05)
ggplot(GSEAres_onco_sig, aes(reorder(pathway, NES), NES, fill = adjPvalue)) +
  geom_col() +
  scale_fill_manual(values = cols) +
  theme(axis.text.x = element_text(angle = 90, vjust = 0.5, hjust=1)) +
  coord_flip() +
  labs(x="Pathway", y="Normalized Enrichment Score",
       title="Oncogenic pathways Enrichment Score from GSEA (p<0.05)")

#Check enrichment of INFgamma gene set at relapse (cf. Nature communications paper)
gmt_files <- list.files(path = bg_path, pattern = '.gmt', full.names = TRUE)
gmt_files
#according to this list KEGG = [11], GO = [53], reactome = [17], wikipathways [21], misigdb [69], this script uses KEGG, GO and Misigdb
bg_genes_INF <- prepare_gmt(gmt_files[70], my_genes, savefile = FALSE)
...
#################################################################################
#Comparison if you rank by raw p-value
#GSEA analysis
# Setting up environment ===================================================
# Clean environment
rm(list = ls(all.names = TRUE)) # will clear all objects including hidden objects
...
gmt_files <- list.files(path = bg_path, pattern = '.gmt', full.names = TRUE)
gmt_files
#according to this list KEGG = [12], GO = [54], reactome = [18], wikipathways [22], misigdb [69], this script uses KEGG, GO and Misigdb
#GO pathways
bg_genes_GO <- prepare_gmt(gmt_files[54], my_genes, savefile = FALSE)

#Prepare ranking of your DF gene
rankings<- sign(df_new$log2FoldChange)*(-log10(df_new$pvalue)) #Here we change to pval instead of adjusted
names(rankings)<-df_new$gene
#Check list
head(rankings)
#Sort genes by signed p value
...
#Check whether it worked
head(GSEAres_GO)
write_csv(GSEAres_GO, "results/GSEA_LMPP_MPP_pt9_GO_pval.csv")
...
#Save
## 5. Save the results -----------------------------------------------
saveRDS(GSEAres_GO, file = "results/GSEA_pt9_BL_REL_LMPP_MPP_GO_pval.rds")
data.table::fwrite(GSEAres_GO, file ="results/GSEA_pt9_BL_REL_LMPP_MPP_GO_pval.tsv" , sep = "\t", sep2 = c("", " ", ""))
...
#Check whether it worked
head(GSEAres_HALLMARK)
write_csv(GSEAres_HALLMARK,"results/Pt9/GSEA_pt9_LMPP_MPP_Hallmark_pval.csv")
...
plotEnrichment(bg_genes_HALLMARK[[head(GSEAres_HALLMARK[order(padj), ], 1)$pathway]],
  ...
  labs(title = head(GSEAres_HALLMARK[order(padj), ], 1)$pathway)
#Save
saveRDS(GSEAres_HALLMARK, file = "results/Pt9/GSEA_pt9_BL_REL_LMPP_MPP_HALLMARK_Pval.rds")
data.table::fwrite(GSEAres_HALLMARK, file ="results/Pt9/GSEA_pt9_BL_REL_LMPP_MPP_HALLMARK_pval.tsv" , sep = "\t", sep2 = c("", " ", ""))
