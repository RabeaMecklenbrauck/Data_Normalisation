...
pt4_BL_REL<-subset(x=pt4_BL_REL, subset=predicted_CellType=='LMPP')
...
res_tbl_all
#Save the table
write.csv(res_tbl_all,"results/Pt4/DEA/DEA_pt4_LMPP.csv" )
...
write.csv(sig_res_all, "results/Pt4/DEA/DEA_pt4_LMPP_padj.csv")
...
## 1. Read in data -----------------------------------------------------------
list.files(in_path)
df_new <- read.csv("results/Pt4/DEA/DEA_pt4_LMPP_padj.csv", row.names = 1)
...
#Check whether it worked
head(GSEAres_GO)
write_csv(GSEAres_GO, "results/Pt4/DEA/GSEA_LMPP_pt4_GO.csv")
...
#Save
## 5. Save the results -----------------------------------------------
saveRDS(GSEAres_GO, file = "results/Pt4/fgsea/GSEA_pt4_BL_REL_LMPP_GO.rds")
data.table::fwrite(GSEAres_GO, file ="results/Pt4/fgsea/GSEA_pt4_BL_REL_LMPP_GO.tsv" , sep = "\t", sep2 = c("", " ", ""))
...
#Check whether it worked
head(GSEAres_KEGG)
write_csv(GSEAres_KEGG,"results/GSEA_pt4_HSC_KEGG.csv")
...
#No relevant pathways found!
# plot the most significantly enriched pathway
#plotEnrichment(bg_genes_KEGG[[head(GSEAres_KEGG[order(padj), ], 1)$pathway]],
              # rankings) + 
  #labs(title = head(GSEAres_KEGG[order(padj), ], 1)$pathway)
#Save
#saveRDS(GSEAres_KEGG, file = "results/GSEA_pt4_BL_REL_LMPP_KEGG.rds")
#data.table::fwrite(GSEAres_KEGG, file ="results/GSEA_pt4_BL_REL_LMPP_KEGG.tsv" , sep = "\t", sep2 = c("", " ", ""))
...
write_csv(GSEAres_HALLMARK,"results/Pt4/fgsea/GSEA_pt4_HSC_Hallmark.csv")
...
#Save
saveRDS(GSEAres_HALLMARK, file = "results/GSEA_pt4_BL_REL_HSC_HALLMARK.rds")
data.table::fwrite(GSEAres_HALLMARK, file ="results/GSEA_pt4_BL_REL_HSC_HALLMARK.tsv" , sep = "\t", sep2 = c("", " ", ""))
