...
#We'll do MEP, if there enough cells
##Create a subset with only dominant population which is MEP in this case
pt4_BL_REL<-subset(x=pt4_BL_REL, subset=predicted_CellType =="MEP")
...
res_tbl_all
#Save the table
write.csv(res_tbl_all,"results/Pt4/DEA/DEA_pt4_MEP.csv" )
...
write.csv(sig_res_all, "results/Pt4/DEA/DEA_pt4_MEP_padj.csv")
...
## 1. Read in data -----------------------------------------------------------
list.files(in_path)
df_new <- read.csv("results/Pt4/DEA/DEA_pt4_MEP_padj.csv", row.names = 1)
...
max(rankings)
min(rankings)
#These are probably not enough genes for sensible analysis, so i"ll try with one
...
#Check whether it worked
head(GSEAres_GO)
write_csv(GSEAres_GO, "results/Pt4/DEA/GSEA_MEP_pt4_GO.csv")
...
#Save
## 5. Save the results -----------------------------------------------
saveRDS(GSEAres_GO, file = "results/Pt4/fgsea/GSEA_pt4_BL_REL_MEP_GO.rds")
data.table::fwrite(GSEAres_GO, file ="results/Pt4/fgsea/GSEA_pt9_BL_REL_MEP_GO.tsv" , sep = "\t", sep2 = c("", " ", ""))

##6.Visualise
GSEAres_GO$adjPvalue <- ifelse(GSEAres_GO$padj <= 0.05, "significant", "non-significant")
cols <- c("non-significant" = "grey", "significant" = "red")
GSEAres_GO_sig=subset(GSEAres_GO, GSEAres_GO$padj<=0.5)
ggplot(GSEAres_GO_sig, aes(reorder(pathway, NES), NES, fill = adjPvalue)) +
  ...
  labs(x="Pathway", y="Normalized Enrichment Score",
       title="GO pathways Enrichment Score from GSEA (p<0.5)")
...
#Check whether it worked
head(GSEAres_KEGG)
write_csv(GSEAres_KEGG,"results/GSEA_pt4_MEP_KEGG.csv")
...
write_csv(GSEAres_HALLMARK,"results/Pt4/fgsea/GSEA_pt4_MEP_Hallmark.csv")
...
#Save
saveRDS(GSEAres_HALLMARK, file = "results/GSEA_pt4_BL_REL_MEP_HALLMARK.rds")
data.table::fwrite(GSEAres_HALLMARK, file ="results/GSEA_pt4_BL_REL_MEP_HALLMARK.tsv" , sep = "\t", sep2 = c("", " ", ""))

#Visualise
GSEAres_HALLMARK$adjPvalue <- ifelse(GSEAres_HALLMARK$padj <= 0.05, "significant", "non-significant")
cols <- c("non-significant" = "grey", "significant" = "red")
GSEAres_HALLMARK_sig=subset(GSEAres_HALLMARK, GSEAres_HALLMARK$padj<=0.5)
ggplot(GSEAres_HALLMARK, aes(reorder(pathway, NES), NES, fill = adjPvalue)) +
  ...
  coord_flip() +
  labs(x="Pathway", y="Normalized Enrichment Score",
       title="Hallmark pathways Enrichment Score from GSEA (p<0.5)")
#No signifkant pathways to plot