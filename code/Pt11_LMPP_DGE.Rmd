---
title: "Pt11_LMPP_Differentialexpression"
...
By projecting the sorted cells of patient 11, we have identified LMPP-like population where the major clone mostly projects to.
Load the data and select Pt11, dominant clone for BL and REL and LMPP-like population.
```{r}
...
```
If everything worked, that should result in 524 LMPPs, 115 for BL and 409 for relapse.
...
Pt11 LMPPs are compared BL against relapse, logFC schrinkage using apeglm and filtered and unfiltered table are stored.
```{r}
...
res_tbl_all
#Save the table
write.csv(res_tbl_all,"results/pt11/results/Deseq2/DEA_pt11_LMPP.csv" )
...
write.csv(sig_res_all, "results/Pt11/results/Deseq2/DEA_pt11_LMPP_padj.csv")
```
...
write_csv(z0, "results/Pt11/results/DREAM/Pt11_DREAM_LMPP.csv")
```
Rund Limma

```{r}
# Make design matrix
table(conditions.df$condition)
as.formula(formula)
design <- model.matrix(~condition, data = conditions.df)
View(design)
colnames(design) <- c("REL", "BL")
## Limma
x = new("EList")
x$E = edgeR::cpm(dge, log=TRUE, prior.count=3)
 fit<-limma::lmFit(object = x, 
                        design = design)
fit <- limma::eBayes(fit, trend = TRUE)
limma::plotSA(fit)
resT <- limma::topTable(fit,coef=2, n = Inf, adjust.method = "BH")
resT <- tibble::rownames_to_column(resT, "Gene")
resT <- resT %>% dplyr::rename(p.value = P.Value, 
                                     adjusted.pval = adj.P.Val, 
                                     log2FC = logFC)
resT_sig = subset (resT, resT$adjusted.pval<0.05)
res_T_up=subset(resT_sig, resT_sig$log2FC>0)
```

```{r}
library(limma)
design<-model.matrix(~pt11_BL_REL$Sample)
