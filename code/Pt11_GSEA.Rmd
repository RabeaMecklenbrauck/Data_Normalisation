...
out_path <- "results/pt11/results/Deseq2"
in_path<-"results/"
...
```{r}
deseq<- read_csv("results/Pt11/results/Deseq2/DEA_pt11_LMPP_padj.csv")
...
head(GSEAres_GO)
write_csv(GSEAres_GO, "results/pt11/results/Deseq2/GSEA_LMPP_pt11_GO.csv")
...
ggsave("GO_pathways_deseq2_pt11_LMPP.pdf", path = "results/Pt11/results/Figures", width =10, height = 10, unit = "cm")

#Save
#Save the results -----------------------------------------------
data.table::fwrite(GSEAres_GO, file ="results/pt11/results/Deseq2/GSEA_pt11_BL_REL_LMPP_GO.tsv" , sep = "\t", sep2 = c("", " ", ""))
```
...
#Check whether it worked
head(GSEAres_KEGG)
write_csv(GSEAres_KEGG,"results/Pt11/results/Deseq2/GSEA_pt11_LMPP_KEGG.csv")
...
ggsave("KEGG_pt11_LMPP_Deseq.pdf", path = "results/Pt11/results/Figures", width =10, height = 10, unit = "cm")
#Save
data.table::fwrite(GSEAres_KEGG, file ="results/Pt11/results/Deseq2/GSEA_pt11_BL_REL_LMPP_KEGG.tsv" , sep = "\t", sep2 = c("", " ", ""))
```
...
write_csv(GSEAres_HALLMARK,"results/Pt11/results/Deseq2/GSEA_pt11_LMPP_HALLMARK.csv")
...
#Save
data.table::fwrite(GSEAres_HALLMARK, file ="results/Pt11/results/Deseq2/GSEA_pt11_BL_REL_LMPP_HALLMARK.tsv" , sep = "\t", sep2 = c("", " ", ""))
```
...
ggplot(GSEAres_HALLMARK_sig , aes(reorder(pathway, NES), NES, fill = )) +
  geom_col() +
  scale_colour_gradient(low = "red", high = "blue", aesthetics = GSEAres_HALLMARK_sig$adjPvalue, guide = "colourbar") +
  theme(axis.text.x = element_text(angle = 90, vjust = 0.5, hjust=1)) +
  coord_flip() +
  labs(x="Pathway", y="Normalized Enrichment Score",
       title="Hallmark pathways Enrichment Score from GSEA (p<0.05)")
ggsave("HALLMAR_pt11_LMPP_Deseq.pdf", path = "results/Pt11/results/Figures", width =10, height = 10, unit = "cm")
```

```{r}
ggplot(GSEAres_HALLMARK_sig, aes(x = NES, y = fct_reorder(pathway, NES))) + 
               geom_point(aes(size = NES, color = GSEAres_HALLMARK_sig$padj)) +
               theme_bw(base_size = 14) +
        scale_colour_gradient(limits=c(0, 0.10), low="red") +
        ylab(NULL) +
        ggtitle("HALLMARK pathway enrichment")
ggsave("HALLMARK_pt11_LMPP_Deseq.pdf", path = "results/Pt11/results/Figures", width =25, height = 18, unit = "cm")
...
#Check whether it worked
head(GSEAres_all)
write_csv(GSEAres_all,"results/Pt11/results/Deseq2/GSEA_pt11_LMPP_pathways.csv")
...
#Save
data.table::fwrite(GSEAres_all, file ="results/Pt11/results/Deseq2/GSEA_pt11_BL_REL_LMPP_all.tsv" , sep = "\t", sep2 = c("", " ", ""))
...
ggsave("allPathways_pt11_LMPP_Deseq.pdf", path = "results/Pt11/results/Figures", width =20, height = 35, unit = "cm")
```
...
head(bg_genes_all)
bg_genes_HALLMARK<-bg_genes_HALLMARK(bg_genes_HALLMARK$gene %in% genes_in_data)
```
...
ggsave("Hallmark_pt11_LMPP_Deseq_dotplot.pdf", path = "results/Pt11/results/Figures", width =20, height = 35, unit = "cm")
```
...
```{r}
enrichplot::dotplot(enrichres_all, x = "geneRatio",showCategory = 25, orderBy = "p.adjust", split=deseq$diffexpr) 
ggsave("Hallmark_pt11_LMPP_Deseq_dotplot_all.pdf", path = "results/Pt11/results/Figures", width =20, height = 35, unit = "cm")
```
