...
pt9_BL_REL<-subset(x=pt9_BL_REL, subset=predicted_CellType %in% c("MPP-MyLy", "MPP-MkEry"))
...
res_tbl_all
#Save the table
write.csv(res_tbl_all,"results/Pt9/DEA_pt9_MPP.csv" )
...
write.csv(sig_res_all, "results/Pt9/DEA_pt9_MPP_padj.csv")
...
## 1. Read in data -----------------------------------------------------------
list.files(in_path)
df_new <- read.csv("results/Pt9/DEA_pt9_MPP_padj.csv", row.names = 1)
...
#Run GSEA
GSEAres <- fgsea(pathways = bg_genes_GO, # List of gene sets to check
                 stats = rankings,
...
#Check whether it worked
head(GSEAres)
write_csv(GSEAres, "results/GSEA_MPP_pt9_GO.csv")
...
#Save
## 5. Save the results -----------------------------------------------
saveRDS(GSEAres, file = "results/GSEA_pt9_BL_REL_MPP_GO.rds")
data.table::fwrite(GSEAres, file ="results/GSEA_pt9_BL_REL_MPP_GO.tsv" , sep = "\t", sep2 = c("", " ", ""))
...
#Check whether it worked
head(GSEAres)
write_csv(GSEAres,"results/GSEA_pt9_MPP_wiki.csv")
...
plotEnrichment(bg_genes[[head(GSEAres[order(padj), ], 1)$pathway]],
  ...
  labs(title = head(GSEAres[order(padj), ], 1)$pathway)
#Save
saveRDS(GSEAres, file = "results/GSEA_pt9_BL_REL_MPP_wiki.rds")

data.table::fwrite(GSEAres, file ="results/GSEA_pt9_BL_REL_MPP_wiki.tsv" , sep = "\t", sep2 = c("", " ", ""))
...
#Check whether it worked
head(GSEAres)
write_csv(GSEAres,"results/GSEA_pt9_MPP_KEGG.csv")
...
plotEnrichment(bg_genes[[head(GSEAres[order(padj), ], 1)$pathway]],
  ...
  labs(title = head(GSEAres[order(padj), ], 1)$pathway)
#Save
saveRDS(GSEAres, file = "results/GSEA_pt9_BL_REL_MPP_KEGG.rds")
data.table::fwrite(GSEAres, file ="results/GSEA_pt9_BL_REL_MPP_KEGG.tsv" , sep = "\t", sep2 = c("", " ", ""))
#Visualise
...
#Check whether it worked
head(GSEAres)
write_csv(GSEAres,"results/GSEA_pt9_MPP_HALLMARK.csv")
...
#Save
saveRDS(GSEAres_HALLMARK, file = "results/Pt9/GSEA_pt9_BL_REL_MPP_HALLMARK.rds")
data.table::fwrite(GSEAres_HALLMARK, file ="results/Pt9/GSEA_pt9_BL_REL_MPP_HALLMARK.tsv" , sep = "\t", sep2 = c("", " ", ""))
#Visualise
GSEAres_HALLMARK$adjPvalue <- ifelse(GSEAres_HALLMARK$padj <= 0.05, "significant", "non-significant")
cols <- c("non-significant" = "grey", "significant" = "red")
GSEAres_KEGG_sig=subset(GSEAres_KEGG, GSEAres_KEGG$padj<=0.3)
...
saveRDS(GSEAres, file = "results/GSEA_pt9_BL_REL_MPP_spikein_GO_noMt.rds")
data.table::fwrite(GSEAres, file ="results/GSEA_pt9_BL_REL_MPP_spikein_GO_noMT.tsv" , sep = "\t", sep2 = c("", " ", ""))
